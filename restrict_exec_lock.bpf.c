...
#define X86_64_FCNTL_SYSCALL 72
#define FCNTL_SYSCALL X86_64_FCNTL_SYSCALL
#define X86_64_FLOCK_SYSCALL 73
#define FLOCK_SYSCALL X86_64_FLOCK_SYSCALL
...
static long check_allow_type_map(struct bpf_map *map, char *key, char *val, struct callback_ctx *data) {
    struct bpf_map *restricted_subject_types = bpf_map_lookup_elem(&allow_type_lock_map, key);
...
static long check_deny_type_map(struct bpf_map *map, char *key, char *val, struct callback_ctx *data) {
    struct bpf_map *restricted_subject_types = bpf_map_lookup_elem(&deny_type_lock_map, key);
    if(restricted_subject_types == NULL)
    	return 0;
...
SEC("lsm/file_lock")
int BPF_PROG(restrict_exec_lock, struct file *file, unsigned int cmd, int ret)
{
    struct pt_regs *regs;
    ...
    if(default_allow) {
        ...
        denied_execs = bpf_map_lookup_elem(&deny_exec_lock_map, e->file_path);
        ...
        allowed_execs = bpf_map_lookup_elem(&allow_exec_lock_map, e->file_path);
