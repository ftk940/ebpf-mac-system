...
struct {
	...
	__type(key, char[MAX_TYPE_LEN]);
	__array(values, struct restricted_groups);
} allow_group_write_map SEC(".maps");
...
static long check_allow_group_map(struct bpf_map *map, char *key, char *val, struct callback_ctx *data) {
    struct bpf_map *all_restricted_groups = bpf_map_lookup_elem(&allow_group_write_map, key);
...
static long check_deny_group_map(struct bpf_map *map, char *key, char *val, struct callback_ctx *data) {
    struct bpf_map *all_restricted_groups = bpf_map_lookup_elem(&deny_group_write_map, key);
    if(all_restricted_groups == NULL)
    	return 0;
...
int BPF_PROG(restrict_user_write, struct file *file, int mask, int ret)
{
    struct pt_regs *regs;
    ...
    if(default_allow) {
        ...
        denied_uids = bpf_map_lookup_elem(&deny_user_write_map, e->file_path);
        ...
        if(denied_uids) {
            ...
            if(v) {
                //bpf_printk("block uid %d write %s\n", uid, file_path);
        ...
        allowed_uids = bpf_map_lookup_elem(&allow_user_write_map, e->file_path);
    ...
    if(default_allow) {
        ...
        if(data.find) {
            /*bpf_printk("block uid %d write %s\n", uid, file_path);
            bpf_printk("group:%s can't write object type:%s\n", data.group, data.object_type);*/
            e->permission_type = DENY;
        ...
        if(data.find) {
            //bpf_printk("group:%s can write object type:%s !!!!!\n", data.group, data.object_type);
            e->permission_type = ALLOW;
            e->restricted_target = GROUP;
            bpf_probe_read_str(e->user_group, sizeof(e->user_group), data.user_group);
            bpf_probe_read_str(e->object_type, sizeof(e->object_type), data.object_type);
