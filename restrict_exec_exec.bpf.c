...
#define X86_64_EXEC_SYSCALL 59
#define EXEC_SYSCALL X86_64_EXEC_SYSCALL
...
/*
// only file, filename, interp contains value 
struct linux_binprm {
...
static long check_allow_type_map(struct bpf_map *map, char *key, char *val, struct callback_ctx *data) {
    struct bpf_map *restricted_subject_types = bpf_map_lookup_elem(&allow_type_exec_map, key);
...
static long check_deny_type_map(struct bpf_map *map, char *key, char *val, struct callback_ctx *data) {
    struct bpf_map *restricted_subject_types = bpf_map_lookup_elem(&deny_type_exec_map, key);
    if(restricted_subject_types == NULL)
    	return 0;
...
int BPF_PROG(restrict_exec_exec, struct linux_binprm *bprm, int ret)
{
    struct pt_regs *regs;
    ...
    struct file *file;
    err = bpf_probe_read_kernel(&file, sizeof(file), &bprm->file);

    char file_path[32] = ""; // MAX_PATH_LEN * 2
    bpf_probe_read_str(&file_path, sizeof(file_path), (void *)bprm->file->f_path.dentry->d_name.name);
    if(mystrcmp(file_path, "a.txt") != 0 && mystrcmp(file_path, "b.txt") != 0)
        return 0;

    //bpf_printk("file: %s\n", file_name);
    /*bpf_probe_read_str(&file_name, sizeof(file_name), (void *)bprm->executable->f_path.dentry->d_name.name);
    bpf_printk("executable: %s\n", file_name);
    bpf_probe_read_str(&file_name, sizeof(file_name), (void *)bprm->interpreter->f_path.dentry->d_name.name);
    bpf_printk("interpreter: %s\n", file_name);

    bpf_probe_read_str(&file_name, sizeof(file_name), (void *)bprm->filename);
    bpf_printk("filename: %s\n", file_name);
    bpf_probe_read_str(&file_name, sizeof(file_name), (void *)bprm->interp);
    bpf_printk("interp: %s\n", file_name);
    bpf_probe_read_str(&file_name, sizeof(file_name), (void *)bprm->fdpath);
    bpf_printk("fdpath: %s\n", file_name);
    */
    ...
    if(default_allow) {
        ...
        denied_execs = bpf_map_lookup_elem(&deny_exec_exec_map, e->file_path);
        ...
        allowed_execs = bpf_map_lookup_elem(&allow_exec_exec_map, e->file_path);
