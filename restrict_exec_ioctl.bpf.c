...
#define X86_64_IOCTL_SYSCALL 16
#define IOCTL_SYSCALL X86_64_IOCTL_SYSCALL
...
static long check_allow_type_map(struct bpf_map *map, char *key, char *val, struct callback_ctx *data) {
    struct bpf_map *restricted_subject_types = bpf_map_lookup_elem(&allow_type_ioctl_map, key);
...
static long check_deny_type_map(struct bpf_map *map, char *key, char *val, struct callback_ctx *data) {
    struct bpf_map *restricted_subject_types = bpf_map_lookup_elem(&deny_type_ioctl_map, key);
    if(restricted_subject_types == NULL)
    	return 0;
...
SEC("lsm/file_ioctl")
int BPF_PROG(restrict_exec_ioctl, struct file *file, unsigned int cmd, unsigned long arg, int ret)
{
    struct pt_regs *regs;
    ...
    if(default_allow) {
        ...
        denied_execs = bpf_map_lookup_elem(&deny_exec_ioctl_map, e->file_path);
        ...
        allowed_execs = bpf_map_lookup_elem(&allow_exec_ioctl_map, e->file_path);
